...
4. **Marquee** — franja con bordes de vidrio, texto mono en `--text-mute` (el contenido es un solo nodo de texto: sin colorear `✱` por separado).
...
2. **Titulares en cascada** — para cada `h1, h2, .about-lead`: JS envuelve cada palabra en su propia máscara `overflow:hidden` (sin re-parentar el marcado inline, `em`/`strong` conservan anidación) y agrupa el stagger por línea leyendo `offsetTop` — las palabras de una misma línea comparten delay, así el efecto visual es línea por línea con `translateY(110%)→0` (~90ms por línea) vía IntersectionObserver. El gradiente del `<em>` se aplica por palabra dividida. Reduced-motion: sin división, reveal estándar.
